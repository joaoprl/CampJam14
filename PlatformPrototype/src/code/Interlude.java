...
public class Interlude {
	...
	public Interlude(String []strings, Point position, float fadeTime, int time, int nextState)
	{
		try{
	...
	{		
		...
		if(waited >= fadeTime * (stringCounter + 1)) stringCounter++;
		...
		if(overlayOpacity < 0) overlayOpacity = 0f;
		if(overlayOpacity > 1) overlayOpacity = 1f;
		if(overlayOpacityGlobal < 0) overlayOpacityGlobal = 0f;
		if(overlayOpacityGlobal > 1) overlayOpacityGlobal = 1f;		
		// if(waited >= totalTime) this.end();
	}
	
	public void draw(Graphics2D g)
	{
		if(g.getFont() != this.font) g.setFont(font);
